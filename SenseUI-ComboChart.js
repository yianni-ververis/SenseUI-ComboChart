...
define(["qlik", "jquery", 'css!./SenseUI-ComboChart.css', './SenseUI-ComboChart-options', "./d3.v3.min", './d3-tip', './SenseUI-ComboChart-css'], function (qlik, $, css, options, d3) {
   ...
   * ***********/
		if (vars.measure2.label && vars.measure2.visible) {
			if (!vars.measure2.type) {
				// if it is a line
				var y2 = d3.scale.linear().range([height, 0]).domain([0, d3.max(vars.data, function (d) {
					return d.measureNum2;
				})]);
				var line = d3.svg.line().x(function (d) {
					return x(d.dimension);
				}).y(function (d) {
					return y2(d.measureNum2);
				});
				// Create the line
				svg.select("#measure2").append("g").attr("id", "line").append("path").datum(vars.data).attr("class", "line2").attr("transform", "translate(" + x.rangeBand() / 2 + ",0)").attr("d", line);
				// Add the dots
   ...
   * ***********/
		if (vars.measure3.label && vars.measure3.visible) {
			if (!vars.measure3.type) {
				// if it is a line
				var y3 = d3.scale.linear().range([height, 0]).domain([0, d3.max(vars.data, function (d) {
					return d.measureNum3;
				})]);
				var line2 = d3.svg.line().x(function (d) {
					return x(d.dimension);
				}).y(function (d) {
					return y3(d.measureNum3);
				});
				// Create the line
				svg.select("#measure3").append("g").attr("id", "line2").append("path").datum(vars.data).attr("class", "line3").attr("transform", "translate(" + x.rangeBand() / 2 + ",0)").attr("d", line2);
				// Add the dots
		...
		if (vars.measure4.label && vars.measure4.visible) {
			if (!vars.measure4.type) {
				...
				svg.select("#measure4").selectAll("dots").data(vars.data).enter().append("circle").attr("class", "dot4").attr("r", vars.measure4.radius).attr("cx", function (d) {
					...
					return y(d.measureNum4);
				}).attr("transform", "translate(" + x.rangeBand() / 2 + ",0)").on('mouseover', function (d, i) {
					tip.show(d, i);
					setTimeout(function () {
						tip.hide();
					}, 10000);
   ...
   * ***********/
		...
		// TOOLTIPS
		var tip = d3.tip().attr('class', vars.id + " d3-tip").offset([-10, 0]).extensionData(vars.tooltip).html(function (d, i) {
			...
			if (vars.measure4.label) {
				var displayMeasure4 = roundNumber(d.measureNum4);
		...
		if (vars.legend) {
			...
			svg.append("foreignObject").attr('width', 500).attr('height', 50).attr("y", "" + (height + 40)).append("xhtml:div").attr("class", "legend").html(displayLegend);
		}
